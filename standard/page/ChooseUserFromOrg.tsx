...
export default class extends React.PureComponent<Props> {
    ...
    componentDidMount() {
        ...
        if (delegate.contact.loadUserOrgTree) {
            ...
            promise = Promise.all([userPromise, orgPromise]).then(([users, orgs]) =>
                this._generateTree(users, orgs)
            );
        }
        return promise
            .then((tree) => {
    ...
    render() {
        ...
        if (tree.length === 1) {
            ...
            const detpPath = tree[0].dept
                ? tree[0].dept.path + '/' + tree[0].dept.code
                : tree[0].path
                  ? tree[0].path
                  : '';
    ...
    _splitSection(children) {
        ...
        return [{ data: notLeafItems, hasFooter: hasFooter }, { data: leafItems }];
    }
        ...
        while (queue.length > 0) {
                ...
                const subdepts = orgs
                    ...
                    .filter((item) =>
                        Array.isArray(excludedUserIds)
                            ? excludedUserIds.indexOf(item.orgId) < 0
                            : true
                    );
            ...
            if (users) {
                const subusers = users
                    ...
                    .filter((item) =>
                        Array.isArray(excludedUserIds)
                            ? excludedUserIds.indexOf(item.userId) < 0
                            : true
                    );
